...
def clean_data(item):
    """
    Limpia los valores NaN e Infinity en el diccionario antes de guardarlo en DynamoDB.
    """
    for key, value in item.items():
        if isinstance(value, (int, float)):
            if math.isnan(value) or math.isinf(value):
                item[key] = None  # O cualquier otro valor adecuado, como 0 o 'Unknown'
    return item
...
def save_to_dynamodb(table_name, item):
    ...
    # Limpiar los valores NaN e Infinity
